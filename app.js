...
if (
  ...
  return console.error(
    "Not found logBucketSetting --> pm2-logrotate-s3-config.json in PM2 home folder"
  );
}
...
if (
  ...
  return console.error(
    "Not found aws credentials --> pm2-logrotate-s3-config.json in PM2 home folder"
  );
}

// Initialize an S3 client with provided credentials
const s3Client = new S3Client({
  region: conf.aws.credentials.region || "ap-south-1", // Specify the AWS region from environment variables
  credentials: {
    accessKeyId: conf.aws.credentials.accessKeyId, // Access key ID from environment variables
    secretAccessKey: conf.aws.credentials.secretAccessKey, // Secret access key from environment variables
  },
});
...
const putFileToS3 = (local_file_path, s3_file_path, s3_bucket) =>
  ...
  fs.readdir(dirName, function (err, files) {
    ...
    for (i = rotated_files.length - 1; i >= RETAIN; i--) {
      (function (i) {
        ...
        const s3_file_path = `${conf.logBucketSetting.s3Path}/${(
...
function proceed(file) {
  ...
  writeStream.on("finish", function () {
    ...
    fs.truncate(file, function (err) {
      ...
      if (typeof RETAIN === "number") putOldFileToS3AndDeletedFromLocal(file);
    });
  });
}
